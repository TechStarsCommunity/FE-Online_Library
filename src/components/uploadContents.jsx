...
{/* when the div is clicked, you can also upload the image */}
        <div className="border rounded-lg border-[#10B2F3] flex flex-col cursor-pointer justify-center items-center w-full h-[200px]" onClick={() => document.getElementById(id).click()}>
        {/* it gets the file name we are uploading and displays it */}
            <input type="file" accept={ accept } hidden id={ id } 
            ...
            <p className="font-bold">{bookCover ? 'Upload jpg or png': 'Upload pdf'}</p>
            </>
            }
        </div>
    </div>
